#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for SmolLM3-3B
=====================================================
...
Note: SmolLM3 supports /think and /no_think modes for extended thinking.
We use /no_think for faster, shorter responses without reasoning chain.
...
# Generation settings
# Using /no_think mode - shorter responses without reasoning chain
...
# 3B model is very small (~6GB), can use large batches
BATCH_SIZE = 24
...
def clean_think_tags(response: str) -> str:
    ...
    SmolLM3 may use these for extended thinking output.
    """
    # Remove think tags and their content
    cleaned = re.sub(r'<think>.*?</think>', '', response, flags=re.DOTALL)
    # Also handle empty think tags
...
def build_messages_smollm3(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for SmolLM3 model with sycophancy evaluation instruction.
    Prepends /no_think to system prompt to disable extended thinking.
    """
    # Add /no_think to disable extended thinking for faster responses
    system_with_no_think = "/no_think\n\n" + system_content
    return [
...
def load_model_and_tokenizer():
    """
    Load SmolLM3-3B with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    """
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages_smollm3(messages[0]["content"], messages[1]["content"])
            )
