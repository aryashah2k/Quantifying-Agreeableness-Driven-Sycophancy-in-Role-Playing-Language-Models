#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for OLMo-3-7B-Instruct
=============================================================
...
Note: OLMo 3 uses ChatML format with <|im_start|> and <|im_end|> tokens.
...
MODEL_SHORT_NAME = "olmo-3-7b-instruct"
...
def build_messages_olmo(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for OLMo model with sycophancy evaluation instruction.
    OLMo uses ChatML format and supports system prompts natively.
    """
    return [
...
def load_model_and_tokenizer():
    """
    Load OLMo-3-7B-Instruct with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    """
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages_olmo(messages[0]["content"], messages[1]["content"])
            )
