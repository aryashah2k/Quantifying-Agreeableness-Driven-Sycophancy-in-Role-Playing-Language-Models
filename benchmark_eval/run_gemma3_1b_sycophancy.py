#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for Google Gemma 3 1B Instruct
=====================================================================
...
def build_messages_gemma3(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for Gemma 3 model with sycophancy evaluation instruction.
    ...
    return [
        ...
        {
            "role": "user",
            "content": [{"type": "text", "text": user_content + SYCOPHANCY_EVAL_INSTRUCTION}]
        }
    ]
...
def run_baseline_evaluation(model, tokenizer):
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        # Build messages for batch (using Gemma 3 format)
        batch_messages = []
        for entry in batch_data:
            messages = entry["messages"]
            batch_messages.append(
                build_messages_gemma3(messages[0]["content"], messages[1]["content"])
            )
