#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for Yi-6B-Chat
=====================================================
...
Note: Yi-6B-Chat is the instruction-tuned version of Yi-6B.
Uses standard chat template with apply_chat_template().
Yi recommends use_fast=False for their tokenizer.
...
# Using Yi-6B-Chat (instruction-tuned) instead of Yi-6B (base model)
MODEL_ID = "01-ai/Yi-6B-Chat"
MODEL_SHORT_NAME = "yi-6b-chat"
...
# 6B model (~12GB), can use moderate batches
BATCH_SIZE = 16
...
def build_messages_yi(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for Yi model with sycophancy evaluation instruction.
    Yi-6B-Chat supports system prompts in chat template.
    """
    return [
...
def load_model_and_tokenizer():
    """
    Load Yi-6B-Chat with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    - use_fast=False for tokenizer (Yi recommendation)
    """
...
def generate_batch(model, tokenizer, batch_messages: list[list[dict]]) -> list[str]:
    ...
    # Apply chat template to get text prompts first
    prompt_texts = []
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages_yi(messages[0]["content"], messages[1]["content"])
            )
