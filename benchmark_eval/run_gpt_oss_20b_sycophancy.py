#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for GPT-OSS-20B
======================================================
...
Note: This model uses OpenAI's harmony response format.
The transformers chat template handles this automatically.
Requires openai-harmony library for parsing responses.
"""
...
# Generation settings
# GPT-OSS uses harmony format with analysis + final channels, needs more tokens
MAX_NEW_TOKENS = 300  # Extra tokens for reasoning chain before final answer
DO_SAMPLE = False     # Greedy decoding for reproducible results

# Batch size for inference (adjust based on VRAM)
# GPT-OSS-20B with MXFP4 quantization fits in ~16GB, A6000 can handle larger batches
BATCH_SIZE = 16
...
# Note: "Reasoning: low" is added to system prompt to reduce chain-of-thought length
...
# Global harmony encoder - initialized once
HARMONY_ENCODER = None


def get_harmony_encoder():
    """Get or initialize the harmony encoder."""
    global HARMONY_ENCODER
    if HARMONY_ENCODER is None:
        HARMONY_ENCODER = load_harmony_encoding(HarmonyEncodingName.HARMONY_GPT_OSS)
    return HARMONY_ENCODER


def parse_harmony_response(output_tokens: list[int]) -> tuple[str, str]:
    """
    Parse GPT-OSS harmony format response using openai-harmony library.
    Returns tuple of (final_response, analysis_response).
    """
    enc = get_harmony_encoder()
    
    try:
        # Parse the completion tokens into messages
        parsed_messages = enc.parse_messages_from_completion_tokens(output_tokens, role=Role.ASSISTANT)
        ...
        for msg in parsed_messages:
            ...
            if hasattr(msg, 'content') and msg.content:
                content = msg.content
                # Check if it has channel info
                if hasattr(content, 'channel'):
                    if content.channel == 'final':
                        final_content = str(content.text) if hasattr(content, 'text') else str(content)
                    elif content.channel == 'analysis':
                        analysis_content = str(content.text) if hasattr(content, 'text') else str(content)
                else:
                    # Try to get text directly
                    final_content = str(content)
        
        return final_content.strip(), analysis_content.strip()
    except Exception as e:
        # Fallback to regex-based parsing if harmony parsing fails
        return "", ""


def clean_harmony_response(response: str) -> str:
    """
    Fallback: Clean GPT-OSS harmony format response using regex.
    Used when token-based parsing is not available.
    """
    # Pattern 1: Look for content after 'final' marker
    final_match = re.search(r'final(.+?)(?:$|analysis|assistant)', response, re.IGNORECASE | re.DOTALL)
    if final_match:
        return final_match.group(1).strip()
    
    # Pattern 2: Split by 'final'
    if 'final' in response.lower():
        parts = re.split(r'final', response, flags=re.IGNORECASE)
        if len(parts) > 1:
            result = parts[-1].strip()
            result = re.sub(r'(assistant|analysis).*$', '', result, flags=re.IGNORECASE).strip()
            return result
    
    # Pattern 3: Split by 'assistant'
...
def build_messages_gpt_oss(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for GPT-OSS model with sycophancy evaluation instruction.
    Adds "Reasoning: low" to system prompt to reduce chain-of-thought length.
    """
    # Add "Reasoning: low" to reduce chain-of-thought length for GPT-OSS
    system_with_reasoning = system_content + "\n\nReasoning: low"
...
def load_model_and_tokenizer():
    ...
    - bfloat16 precision
    - eager attention (SDPA may not be compatible)
    - Dynamic KV-cache
    
    Note: GPT-OSS uses harmony response format, handled by chat template.
    """
    ...
    # Try native support first, fall back to trust_remote_code if needed
    try:
        ...
        print("Loaded using native transformers support")
    except Exception as e:
...
def generate_batch(model, tokenizer, batch_messages: list[list[dict]]) -> list[dict]:
    """
    Generate responses for a batch of prompts.
    Returns list of dicts with raw_response, final_response, analysis, score, and stance.
    """
    ...
    for output in outputs:
        ...
        # Parse using harmony library
        final_response, analysis = parse_harmony_response(generated_ids)
        
        # Decode raw response for logging
        raw_response = tokenizer.decode(generated_ids, skip_special_tokens=False)
        
        # Extract sycophancy score from final response
        score, stance = extract_sycophancy_score(final_response)
        
        if score is None:
            # Fallback: try on cleaned raw response
            cleaned = clean_harmony_response(tokenizer.decode(generated_ids, skip_special_tokens=True))
            score, stance = extract_sycophancy_score(cleaned)
            if final_response == "":
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages_gpt_oss(messages[0]["content"], messages[1]["content"])
            )
        ...
        for entry, response in zip(batch_data, responses):
            result = {
                ...
                "extracted_stance": response["extracted_stance"],
            ...
            if response["sycophancy_score"] is not None:
                category_scores[category]["scores"].append(response["sycophancy_score"])
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for result in all_results:
            writer.writerow([
                ...
                result["final_response"][:500]  # Truncate for CSV
            ])
...
def run_persona_evaluation(model, tokenizer):
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry, response in zip(batch_data, responses):
            ...
            if response["sycophancy_score"] is not None:
                persona_scores[persona_id]["scores"].append(response["sycophancy_score"])
                category = entry["category"]
                if category not in persona_scores[persona_id]["by_category"]:
                    persona_scores[persona_id]["by_category"][category] = []
                persona_scores[persona_id]["by_category"][category].append(response["sycophancy_score"])
