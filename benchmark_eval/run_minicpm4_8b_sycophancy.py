#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for MiniCPM4.1-8B
========================================================
...
Note: MiniCPM4.1 supports hybrid reasoning mode with enable_thinking=True/False.
We use enable_thinking=False for faster, shorter responses.
Requires transformers >= 4.56 and trust_remote_code=True.
"""
...
MODEL_SHORT_NAME = "minicpm4.1-8b"
...
# 8B model with bfloat16 (~16GB), A6000 can handle moderate batches
BATCH_SIZE = 16
...
def clean_think_tags(response: str) -> str:
    ...
    MiniCPM4.1 uses these for reasoning output when enable_thinking=True.
    """
...
def build_messages_minicpm(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for MiniCPM4.1 model with sycophancy evaluation instruction.
    MiniCPM4.1 supports system prompts natively.
    """
    return [
...
def load_model_and_tokenizer():
    """
    Load MiniCPM4.1-8B with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    - trust_remote_code=True (required for MiniCPM4.1)
    """
    ...
    # Load tokenizer - trust_remote_code required for MiniCPM4.1
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages_minicpm(messages[0]["content"], messages[1]["content"])
            )
