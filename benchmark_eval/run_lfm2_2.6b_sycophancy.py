#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for LFM2-2.6B
====================================================
...
Note: LFM2 uses a ChatML-like chat template with <|im_start|> and <|im_end|>.
Supports system prompts. Requires transformers >= 4.55.
Small model (2.6B), allows large batch sizes.
"""
...
# 2.6B model is small (~5GB), can use large batches
BATCH_SIZE = 64
...
def build_messages_lfm2(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for LFM2 model with sycophancy evaluation instruction.
    LFM2 uses ChatML-like format and supports system prompts.
    """
    return [
...
def load_model_and_tokenizer():
    """
    Load LFM2-2.6B with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    """
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages_lfm2(messages[0]["content"], messages[1]["content"])
            )
