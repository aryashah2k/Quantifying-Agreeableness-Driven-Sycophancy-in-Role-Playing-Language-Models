#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for Phi-4-mini-instruct
==============================================================
...
MODEL_SHORT_NAME = "phi4-mini-instruct"
...
# RTX A6000 (48GB) with ~3.8B model can handle larger batches
BATCH_SIZE = 24
...
def build_messages(system_content: str, user_content: str) -> list[dict]:
    """Build chat messages with sycophancy evaluation instruction."""
    return [
...
def load_model_and_tokenizer():
    """
    Load Phi-4-mini-instruct with optimizations:
    - bfloat16 precision
    - SDPA attention (default in PyTorch 2.1+)
    - Dynamic KV-cache
    - trust_remote_code=True (required for Phi-4 on older transformers)
    """
    ...
    # Use native Phi3 architecture support (transformers >= 4.49.0 has native Phi-4 support)
    # If using older transformers, try without trust_remote_code first
    try:
        model = AutoModelForCausalLM.from_pretrained(
            ...
            trust_remote_code=False,  # Try native support first
        )
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        for entry in batch_data:
            ...
            batch_messages.append(
                build_messages(messages[0]["content"], messages[1]["content"])
            )
