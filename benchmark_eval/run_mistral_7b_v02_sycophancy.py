#!/usr/bin/env python3
"""
Sycophancy Benchmark Inference Script for Mistral-7B-Instruct-v0.2
===================================================================
...
Note: Mistral-7B-Instruct-v0.2 uses [INST] and [/INST] tokens.
The chat template does not support system prompts natively,
so we prepend the system instruction to the user message.
"""
...
MODEL_SHORT_NAME = "mistral-7b-instruct-v0.2"
...
# Generation settings
MAX_NEW_TOKENS = 150  # Single sentence response with reasoning
DO_SAMPLE = False     # Greedy decoding for reproducible results

# Batch size for inference (adjust based on VRAM)
# 7B model with bfloat16 (~14GB), A6000 can handle larger batches
BATCH_SIZE = 16
...
def build_messages_mistral(system_content: str, user_content: str) -> list[dict]:
    """
    Build chat messages for Mistral model with sycophancy evaluation instruction.
    Mistral-7B-Instruct-v0.2 doesn't support system prompts natively,
    so we prepend the system instruction to the user message.
    """
    # Combine system and user prompt since Mistral v0.2 doesn't support system role
    combined_prompt = f"{system_content}\n\n{user_content}{SYCOPHANCY_EVAL_INSTRUCTION}"
...
def load_model_and_tokenizer():
    """
    Load Mistral-7B-Instruct-v0.2 with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    """
    ...
    # Set pad token if not set (Mistral uses eos_token)
...
def run_baseline_evaluation(model, tokenizer):
    ...
    for batch_start in tqdm(range(0, len(data), BATCH_SIZE), desc="Processing", total=num_batches):
        ...
        # Build messages for batch (using Mistral format - no system role)
        batch_messages = []
        for entry in batch_data:
            messages = entry["messages"]
            batch_messages.append(
                build_messages_mistral(messages[0]["content"], messages[1]["content"])
            )
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for result in all_results:
            writer.writerow([
                ...
                result["raw_response"][:500]  # Truncate for CSV
            ])
