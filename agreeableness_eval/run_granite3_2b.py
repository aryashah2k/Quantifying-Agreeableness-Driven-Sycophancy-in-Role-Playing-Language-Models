#!/usr/bin/env python3
"""
Agreeableness Questionnaire Inference Script for Granite-3.3-2B-Instruct
========================================================================
...
Note: Granite 3.3 supports thinking=True/False in apply_chat_template().
Uses <think></think> and <response></response> tags for structured reasoning.
We use thinking=False for faster, shorter responses.
"""
...
MODEL_SHORT_NAME = "granite-3.3-2b-instruct"
...
# Generation settings
# Using thinking=False mode for shorter responses
...
# 2B model is small (~4GB), can use large batches
BATCH_SIZE = 24
...
def clean_think_tags(response: str) -> str:
    """
    Remove <think>...</think> and <response>...</response> tags from response.
    Granite uses these for structured reasoning output.
    """
    # Remove think tags and their content
    cleaned = re.sub(r'<think>.*?</think>', '', response, flags=re.DOTALL)
    # Remove response tags but keep content
...
def extract_score(response: str) -> int | None:
    ...
    """
    # First clean any think/response tags
    ...
    for pattern in patterns:
        match = re.search(pattern, clean_response, re.IGNORECASE | re.MULTILINE)
        if match:
            return int(match.group(1))
    
    # Fallback: first standalone digit 1-5 in cleaned response
    fallback_match = re.search(r'\b([1-5])\b', clean_response)
    if fallback_match:
...
def build_prompt(persona: str, question_text: str, prompt_template: dict) -> list[dict]:
    """Build chat messages for Granite model."""
...
def load_model_and_tokenizer():
    """
    Load Granite-3.3-2B-Instruct with optimizations:
    - bfloat16 precision
    - SDPA attention
    - Dynamic KV-cache
    """
...
def generate_single(model, tokenizer, messages: list[dict]) -> str:
    ...
    # Use thinking=False for shorter, faster responses
...
def generate_batch(model, tokenizer, batch_messages: list[list[dict]]) -> list[str]:
    ...
    # Apply chat template to get text prompts first (thinking=False)
    prompt_texts = []
