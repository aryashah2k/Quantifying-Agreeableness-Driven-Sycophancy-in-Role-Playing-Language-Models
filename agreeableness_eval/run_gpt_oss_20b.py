#!/usr/bin/env python3
"""
Agreeableness Questionnaire Inference Script for GPT-OSS-20B
=============================================================
...
def extract_score_from_text(text: str) -> int | None:
    """
    Extract the 1-5 score from text.
    ...
    patterns = [
        ...
        r"\b([1-5])\b",  # Any standalone digit 1-5 (fallback)
    ]
...
def generate_single(model, tokenizer, messages: list[dict]) -> dict:
    """
    Generate response for a single prompt.
    Returns dict with raw_response, final_response, analysis, and extracted_score.
    """
    ...
    # Extract score from final response, fallback to regex on raw
    score = extract_score_from_text(final_response)
    if score is None:
        # Fallback: try regex on cleaned raw response
        cleaned = clean_harmony_response(tokenizer.decode(generated_ids, skip_special_tokens=True))
        score = extract_score_from_text(cleaned)
        if final_response == "":
...
def generate_batch(model, tokenizer, batch_messages: list[list[dict]]) -> list[dict]:
    ...
    Returns list of dicts with raw_response, final_response, analysis, and extracted_score.
    """
    ...
    for ids in input_ids_list:
        original_lengths.append(ids.shape[0])
...
def run_inference():
    ...
    for persona_idx, persona in enumerate(persona_pbar):
        ...
        for batch_start in question_pbar:
            ...
            # Generate responses (now returns list of dicts with parsed harmony format)
            if len(batch_messages) == 1:
                results = [generate_single(model, tokenizer, batch_messages[0])]
            else:
                results = generate_batch(model, tokenizer, batch_messages)
            
            # Process responses
            for q, result in zip(batch_questions, results):
                persona_responses.append({
                    ...
                    "final_response": result["final_response"],  # Parsed final channel
                    "analysis": result["analysis"],  # Parsed analysis/reasoning
