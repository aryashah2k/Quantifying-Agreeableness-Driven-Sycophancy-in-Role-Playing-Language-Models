#!/usr/bin/env python3
"""
Agreeableness Questionnaire Inference Script for Llama-3.1-8B-Instruct
======================================================================
Optimized for RTX A6000 (48GB VRAM) without Flash Attention.
...
MODEL_SHORT_NAME = "llama3.1-8b-instruct"
...
# Generation settings
MAX_NEW_TOKENS = 100  # Short responses expected (number + justification)
DO_SAMPLE = False     # Greedy decoding for reproducible results (avoids nan/inf with low temp)

# Batch size for inference (adjust based on VRAM)
# RTX A6000 (48GB) with 8B model (~16GB) can handle larger batches
BATCH_SIZE = 16
...
def extract_score(response: str) -> int | None:
    ...
    """
    # Look for patterns like "1", "2", "3", "4", "5" at start or standalone
    # Also handles "Rating: 4" or "Score: 3" patterns
    patterns = [
        r"^[\s]*([1-5])[\s\.\:\,]",  # Score at beginning
...
def load_model_and_tokenizer():
    """
    Load Llama 3.1 8B Instruct with optimizations:
    - bfloat16 precision
    - SDPA attention (default in PyTorch 2.1+)
    - Dynamic KV-cache
    """
    ...
    # Set pad token if not set (use eos_token for Llama)
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="cuda:0",  # Use GPU 1 (GPU 0 is occupied)
...
def generate_single(model, tokenizer, messages: list[dict]) -> str:
    ...
    with torch.inference_mode():
        outputs = model.generate(
            ...
            use_cache=True,  # Dynamic KV-cache
        )
...
def generate_batch(model, tokenizer, batch_messages: list[list[dict]]) -> list[str]:
    """Generate responses for a batch of prompts using tokenizer's native padding."""
...
if __name__ == "__main__":
    ...
    # Disable torch.compile inductor to avoid CUDA assertion errors
